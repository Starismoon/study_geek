...
func NewUserHandler(svc service.UserService,
	hdl ijwt.Handler,
	codeSvc service.CodeService, l logger.LoggerV1) *UserHandler {
	return &UserHandler{
...
func (h *UserHandler) LoginSMS(ctx *gin.Context) {
	var err error
	defer h.LogRecord("LoginSMS ", &err)
	type Req struct {
...
func (h *UserHandler) SendSMSLoginCode(ctx *gin.Context) {
	var err error
	defer h.LogRecord("SendSMSLoginCode ", &err)
	type Req struct {
		Phone string `json:"phone"`
	}
	var req Req
...
func (h *UserHandler) LoginJWT(ctx *gin.Context) {
	var err error
	defer h.LogRecord("LoginJWT ", &err)
	type Req struct {
		Email    string `json:"email"`
		Password string `json:"password"`
	}
...
func (h *UserHandler) RefreshToken(ctx *gin.Context) {
	var err error
	defer h.LogRecord("RefreshToken ", &err)
...
func (h *UserHandler) LogoutJWT(ctx *gin.Context) {
	var err error
	defer h.LogRecord("LogoutJWT ", &err)
...
func (h *UserHandler) LogRecord(msg string, err *error) {
	if err != nil {
		h.l.Error(msg, logger.Field{
			Key: "err",
			Val: *err,
		})
	}
